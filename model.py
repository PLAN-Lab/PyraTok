...
class AutoencoderKLPyraTok(ModelMixin, AutoencoderMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        lapq_num_quantizers: int = 2,
        lapq_codebook_dim: int | None = 16,
        lapq_commitment_weight: float = 0.25,
        lapq_entropy_weight: float = 0.1,
        ...
        lapq_text_embed_dim: int | None = None,
        lapq_text_mlp_hidden_dim: int | None = None,
        lapq_text_condition_heads: int | None = None,
        lapq_text_condition_scale: float = 1.0,
    ) -> None:
        super().__init__()
        ...
        self.quant_conv = PyraTokCausalConv3d(z_dim * 2, z_dim * 2, 1)
        self.post_quant_conv = PyraTokCausalConv3d(z_dim, z_dim, 1)
        self.use_lapq_quantizer = bool(use_lapq_quantizer)
        self.lapq_text_input_dim = lapq_text_input_dim
        self.lapq_text_embed_dim = lapq_text_embed_dim
        self.lapq_text_mlp_hidden_dim = lapq_text_mlp_hidden_dim

        # Optional internal MLP to downscale text embeddings (e.g. 2560 -> 256)
        # before they are used as K/V context in LaPQ.
        if (
            self.use_lapq_quantizer
            and lapq_text_input_dim is not None
            and lapq_text_embed_dim is not None
            and lapq_text_input_dim != lapq_text_embed_dim
        ):
            hidden_dim = (
                lapq_text_mlp_hidden_dim
                if lapq_text_mlp_hidden_dim is not None
                else max(lapq_text_embed_dim * 2, lapq_text_embed_dim)
            )
            self.lapq_text_mlp = nn.Sequential(
                nn.Linear(lapq_text_input_dim, hidden_dim),
                nn.SiLU(),
                nn.Linear(hidden_dim, lapq_text_embed_dim),
            )
        else:
            self.lapq_text_mlp = nn.Identity()

        self.lapq_quantizer = None
        # if self.use_lapq_quantizer:
        self.lapq_quantizer = ResidualLaPQunatizer3D(
                input_dim=z_dim,
                num_codes=lapq_num_codes,
                num_quantizers=lapq_num_quantizers,
                commitment_weight=lapq_commitment_weight,
                entropy_weight=lapq_entropy_weight,
                inv_temperature=lapq_inv_temperature,
                codebook_dim=lapq_codebook_dim,
                quantize_dropout=lapq_quantize_dropout,
                quantize_dropout_cutoff_index=lapq_quantize_dropout_cutoff_index,
                quantize_dropout_multiple_of=lapq_quantize_dropout_multiple_of,
                text_embed_dim=lapq_text_embed_dim,
                text_condition_heads=lapq_text_condition_heads,
                text_condition_scale=lapq_text_condition_scale,
            )
        self.last_lapq_loss = None
        self.last_lapq_perplexity = None
        self.last_lapq_indices = None

        self.decoder = PyraTokDecoder3d(
        ...
        self._cached_conv_counts = {
            "decoder": sum(isinstance(m, PyraTokCausalConv3d) for m in self.decoder.modules())
            if self.decoder is not None
            else 0,
            "encoder": sum(isinstance(m, PyraTokCausalConv3d) for m in self.encoder.modules())
            if self.encoder is not None
            else 0,
        }

    def _maybe_quantize_latents(
        self,
        z: torch.Tensor,
        text_embeds: torch.Tensor | None = None,
        text_attention_mask: torch.Tensor | None = None,
        use_lapq_quantizer: bool | None = None,
    ) -> torch.Tensor:
        if use_lapq_quantizer is None:
            use_lapq_quantizer = self.use_lapq_quantizer

        if not use_lapq_quantizer:
            self.last_lapq_loss = None
            self.last_lapq_indices = None
            return z

        if self.lapq_quantizer is None:
            raise RuntimeError(
                "LaPQ quantizer is not initialized. Set `use_lapq_quantizer=True` in the model config to enable it."
            )

        if text_embeds is not None:
            if self.lapq_text_input_dim is not None and text_embeds.shape[-1] != self.lapq_text_input_dim:
                raise ValueError(
                    f"text_embeds last dim ({text_embeds.shape[-1]}) does not match "
                    f"expected lapq_text_input_dim ({self.lapq_text_input_dim})."
                )

            if isinstance(self.lapq_text_mlp, nn.Identity):
                text_embeds_proj = text_embeds
            else:
                mlp_weight = self.lapq_text_mlp[0].weight
                text_embeds_proj = self.lapq_text_mlp(
                    text_embeds.to(device=mlp_weight.device, dtype=mlp_weight.dtype)
                )

            if self.lapq_text_embed_dim is not None and text_embeds_proj.shape[-1] != self.lapq_text_embed_dim:
                raise ValueError(
                    f"Projected text_embeds last dim ({text_embeds_proj.shape[-1]}) does not match "
                    f"lapq_text_embed_dim ({self.lapq_text_embed_dim})."
                )
            text_embeds = text_embeds_proj.to(device=z.device, dtype=z.dtype)

        z_q, q_loss, q_indices = self.lapq_quantizer(
            z,
            text_embeds=text_embeds,
            text_attention_mask=text_attention_mask,
        )
        self.last_lapq_loss = q_loss
        # self.last_lapq_perplexity = q_perplexity
        self.last_lapq_indices = q_indices
        return z_q
    ...
    def quantize(
        ...
        return self._maybe_quantize_latents(
            z,
            text_embeds=text_embeds,
            text_attention_mask=text_attention_mask,
            use_lapq_quantizer=use_lapq_quantizer,
        )
    ...
    def forward(
    ...
    ) -> AutoencoderKLPyraTokForwardOutput | DecoderOutput | tuple[torch.Tensor]:
        """
        Args:
        ...
        z = posterior.mode()
        z = self._maybe_quantize_latents(
            z,
            text_embeds=text_embeds,
            text_attention_mask=text_attention_mask,
            use_lapq_quantizer=use_lapq_quantizer,
        )
        quantization_loss = self.last_lapq_loss

        dec = self.decode(z, return_dict=True).sample

        if not return_dict:
            return (dec,)

        if return_loss_details:
            return AutoencoderKLPyraTokForwardOutput(
                sample=dec,
                quantization_loss=quantization_loss,
                lapq_indices=self.last_lapq_indices,
            )
