...
class LTXPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    def __call__(
        ...
        image = self.video_processor.preprocess_video(image, height=height, width=width)
        image = image.to(device=device, dtype=prompt_embeds.dtype)

        num_frames = image.shape[2]

        num_channels_latents = self.transformer.config.in_channels
        latents2 = self.prepare_latents2(
            image,
                ...
                if edit:
                    if i>1 and i < 45:
                        ...
                        with torch.enable_grad():
                            ...
                            optimizer = torch.optim.Adam([opt_latents], lr=0.01)
                            t = [decode_timestep] * batch_size
                            for _ in range(optimization_steps):
                                latents_p = self.scheduler.one_step_final(noise_pred, t, opt_latents, return_dict=False)[0]
                                # print(latents_p.shape)

                                # vp = self._unpack_latents(
                                #             latents_p,
                                #             latent_num_frames,
                                #             latent_height,
                                #             latent_width,
                                ...
                                # vp = self._denormalize_latents(
                                #     vp, self.vae.latents_mean, self.vae.latents_std, self.vae.config.scaling_factor
                                # )
                                # vp = vp.to(prompt_embeds.dtype)
                                

                                # temb = torch.tensor(t, device=device, dtype=latents.dtype)
                                # frames = self.vae.decode(vp, temb, return_dict=False)[0]

                                # print(image.shape, frames.shape)

                                loss = (1000*torch.nn.functional.mse_loss(latents_p, latents2, reduction="none")).mean()
                                loss.backward()
                                optimizer.step()
                                # grad = torch.autograd.grad(loss, opt_latents)[0]
                                # print(grad.shape)
                                # grad = torch.clamp(grad, -0.5, 0.5)
                                # opt_latents = opt_latents - 0.1 * grad
                                # loss.backward()
